...
然后 Task 回调 `pm`。达到 3 次则按章程 abort。

## 被回调

frontend 反馈 API 不够时，只扩展 `algorithm.js` 和相关测试，不重写无关前端。修复后 handoff 回 `frontend`。

QA 反馈算法缺陷时，先复现，再补测试，修复后 handoff 回 `qa`。
