...
2. 判断需求是否适合本仓库用前端静态动画实现。拒绝高风险、过宽、不可验证、依赖后端或外部私有数据的需求。
   ...
   - 如果需求包含 `### 继续修复来源`，必须在 PRD 中记录原 Issue / PR，并增加“上次修复失败原因假设”和“本次回归验证步骤”。

3. 可行时写 `$PRD_PATH`，必须包含：
...
4. 完成 PRD 后执行。新算法需求交给 algorithm；auto-fix 若不涉及算法逻辑，可直接把 `--owner` 和 `--to` 设为 `frontend`：
   ...
   ```bash
   AGENT_ROLE=产品经理 bash scripts/update-status.sh --stage prd_done --owner algorithm --from pm --to algorithm --artifact "$PRD_PATH" --message "PRD is ready."
   AGENT_ROLE=产品经理 bash scripts/feishu.sh handoff 产品经理 算法工程师 "$PRD_PATH" "  - 已定义算法边界\n  - 已列出可视化步骤\n  - 已写明验收清单"
   ```
...
不可行时写 `$DECISION_PATH`，说明拒绝原因和可接受的改写方式，然后执行：

```bash
AGENT_ROLE=产品经理 bash scripts/update-status.sh --stage rejected --owner pm --from pm --to system --artifact "$DECISION_PATH" --message "Request rejected by PM."
...
如果 algorithm 或 frontend 因 PRD 模糊回调你，补充 `$PRD_PATH` 的澄清章节，更新状态并把 `current_owner` 改回原 agent 后退出。不要把已经可以由 algorithm/frontend/qa 判断的问题重新扩大化。
