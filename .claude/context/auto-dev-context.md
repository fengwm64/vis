...
- 卡片内容区域必须保留清晰的顶部留白。不要在 `CardContent`、`CardFooter` 或动画页面容器上使用 `pt-0` / `padding-top: 0`；新动画建议使用 `p-4`、`p-5` 或 `p-6` 这类包含顶部 padding 的类。
- `npm run build` 必须通过。
...
- 构建失败、路由缺失、交互控件缺失、无用/冗余按钮、死按钮、文案行为不一致、播放控制 bug、卡片顶部留白缺失、视觉验收失败：QA 回调 frontend。
